...
int VL6180_RdMulti(VL6180Dev_t dev, uint16_t index, uint8_t *data, int nData)
{
	...
	reg[0] = (index >> 8)  & 0xFF;
	reg[1] = (index >> 0)  & 0xFF;
	status_int = i2c_write_read(dev->i2c, dev->I2cDevAddr, reg, 2, data, nData);
	if (status_int < 0) {
...
int VL6180_RdByte(VL6180Dev_t dev, uint16_t index, uint8_t *data)
{
	...
	reg[0] = (index >> 8)  & 0xFF;
	reg[1] = (index >> 0)  & 0xFF;
	status_int = i2c_write_read(dev->i2c, dev->I2cDevAddr, reg, 2, data, 1);
	if (status_int < 0) {
		Status = VL6180X_ERROR_CONTROL_INTERFACE;
...
{ 
	...
	reg[0] = (index >> 8)  & 0xFF;
	reg[1] = (index >> 0)  & 0xFF;
	status_int = i2c_write_read(dev->i2c, dev->I2cDevAddr, reg, 2, buf, 2);
	if (status_int < 0) {
...
int VL6180_RdDWord(VL6180Dev_t dev, uint16_t index, uint32_t *data)
{
	...
	reg[0] = (index >> 8)  & 0xFF;
	reg[1] = (index >> 0)  & 0xFF;
	status_int = i2c_write_read(dev->i2c, dev->I2cDevAddr, reg, 2, buf, 4);
	if (status_int < 0) {
		LOG_ERR("i2c_burst_read failed");
		return -EIO;
	}
